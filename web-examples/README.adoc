...
only difference is that it also serves an angular app.
...
This example shows a music store implemented with angular js, where all album data and orders are taken in realtime.
...
== Programmatic Auth example

This example shows how to use the Auth Handlers to protect resources by asserting if the user has the right authorities
to access the resource.

The link:src/main/java/io/vertx/example/web/custom_auth/[Java custom_auth example]
