...
pool.begin(function (res1, res1_err) {
  ...
  // create a test table
  tx.query("create table test(id int primary key, name varchar(255))").execute(function (res2, res2_err) {
    if (res2_err != null) {
      tx.close();
    ...
    tx.query("insert into test values (1, 'Hello'), (2, 'World')").execute(function (res3, res3_err) {

      // rollback transaction
      tx.rollback(function (res4, res4_err) {

        // query some data with arguments
        pool.query("select * from test").execute(function (rs, rs_err) {
          if (rs_err != null) {
