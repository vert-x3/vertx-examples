...
This requires to use the Ceylon repository snapshot: add this remote Ceylon repo containing the requires modules for running Vert.x https://vertx.ci.cloudbees.com/view/vert.x-3/job/vert.x3-lang-ceylon/ws/target/repo/

Edit the file `~/.ceylon/config` and make it look like:

----
[repositories]
remote=https://vertx.ci.cloudbees.com/view/vert.x-3/job/vert.x3-lang-ceylon/ws/target/repo/
----

Or use `ceylon config` if you have Ceylon installed.
