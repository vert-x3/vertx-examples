...
import io.vertx.ceylon.web.templ { handlebarsTemplateEngine }
...
shared class Server() extends Verticle() {
  ...
  shared actual void start() {
    ...
    value engine = handlebarsTemplateEngine.create();
    ...
    router_.get().handler {
      void requestHandler(RoutingContext ctx) {
        ...
        //
        engine.render(ctx, "templates/index.hbs", (Buffer|Throwable res) {
          switch(res)
          case (is Buffer) {
