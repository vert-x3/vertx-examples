...
----
mvn clean package
java -jar target/jgroups-examples-3.3.0.CR1-fat.jar run io.vertx.example.jgroups.ProducerApp -cluster -cp . \
    -Djgroups.bind_addr=127.0.0.1
java -jar target/jgroups-examples-3.3.0.CR1-fat.jar run io.vertx.example.jgroups.ConsumerApp -cluster -cp . \
    -Djgroups.bind_addr=127.0.0.1
----
