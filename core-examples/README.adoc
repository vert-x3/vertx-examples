...
The `GatewayVerticle` deploys two instances of the `HttpServerVerticle` verticle.
...
link:src/main/java/io/vertx/example/core/ha/Server.java[The server]
link:src/main/java/io/vertx/example/core/ha/BareInstance.java[The bare instance]
...
* Start the server by executing the `main` method of the `GatewayVerticle` class
...
In a terminal, find the process related to the `GatewayVerticle` class execution and kill it using `kill -9`. The verticle is
